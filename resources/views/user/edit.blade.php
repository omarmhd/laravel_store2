...
<div class="container bootstrap snippet" style="margin-top: 50px;">
  ...
  <div class="alert alert-warning" role="alert"> <strong>تنبيه!</strong>{{ session('status') }}</div>
  @endif
  @if(session('success')!==null)
  <div class="alert alert-success" role="alert"> <strong>تهانينا !</strong>{{ session('success') }}</div>
  @endif
  <div class="row">
          ...
          <div class="tab-pane active" id="home">
            ...
            <form class="form" action="{{route('client.update')}}" method="post" id="registrationForm">
                @method('put')
                  ...
                  <div class="col-xs-12">
                    <label for="city"><h4> المنطقة</h4></label>
                     <input type="text" class="form-control"  name="city" value="{{$user->billing_city}}" id="city" placeholder="المنطقة السكنية" title="المنطقة السكنية">
                 </div>
             </div>
                  <div class="col-xs-12">
                      <label for="place"><h4> مكان السكن</h4></label>
                      <input type="text" class="form-control" name="place" value="{{$user->billing_address}}" id="place" placeholder="ادخل عنوانك بالتفصيل" title="ادخل عنوانك بالتفصيل">
                  </div>
              </div>
